...
DEVICE_PROP Device_Property = { HID_init, HID_Reset, HID_Status_In, HID_Status_Out, HID_Data_Setup, HID_NoData_Setup,
		HID_Get_Interface_Setting, HID_GetDeviceDescriptor, HID_GetConfigDescriptor, HID_GetStringDescriptor, 0, 0x40 /*MAX PACKET SIZE*/
};
...
ONE_DESCRIPTOR Device_Descriptor = { (uint8_t*) RHID_DeviceDescriptor,
RHID_SIZ_DEVICE_DESC };

ONE_DESCRIPTOR Config_Descriptor = { (uint8_t*) RHID_ConfigDescriptor,
RHID_SIZ_CONFIG_DESC };

ONE_DESCRIPTOR RHID_Report_Descriptor = { (uint8_t *) RHID_ReportDescriptor,
RHID_SIZ_REPORT_DESC };

ONE_DESCRIPTOR RHID_Hid_Descriptor = { (uint8_t*) RHID_ConfigDescriptor + RHID_OFF_HID_DESC,
RHID_SIZ_HID_DESC };

ONE_DESCRIPTOR String_Descriptor[4] = { { (uint8_t*) RHID_StringLangID, RHID_SIZ_STRING_LANGID }, {
		(uint8_t*) RHID_StringVendor, RHID_SIZ_STRING_VENDOR },
		{ (uint8_t*) RHID_StringProduct, RHID_SIZ_STRING_PRODUCT }, { (uint8_t*) RHID_StringSerial,
...
void HID_Status_In(void) {
	...
	// Invoke when data was sending via SET_FEATURE method

	trace_printf("HID Status IN ");
	trace_printf("Buffer: %u, %u, %u, %u \r\n", report_buf[0], report_buf[1], report_buf[2], report_buf[3]);
	uint8_t* ptr;
	ptr = Buffer;
	...
	case 2:
		...
		break;
	case 3:
		if (report_buf[1] == CMD_READ_AUTH || CMD_READ_HOST || CMD_READ_SSID || CMD_READ_SSID_PASS)
			requestCommand = report_buf[1];
	}
}
